...
def create_resource_skeleton(resource_class: str, instance_num: int) -> Tuple[str, Dict[str, Any]]:
    """Create a resource skeleton with all properties as comments or placeholders"""
    ...
    for prop_name, prop_info in properties.items():
        ...
        if prop_type == "dict" and prop_name == "match":
            # For match dictionaries, provide common examples
            resource_config[prop_name] = {
                "@SUBSYSTEM": "usb",  # Common example
                "@ID_SERIAL_SHORT": "CHANGEME"
            }
        elif prop_type == "int":
            if "port" in prop_name.lower() or "index" in prop_name.lower():
        ...
        # Add comment about the property if not required
        if not required:
            # This is a placeholder - in real YAML output we'll add comments
            pass
    
    return label, resource_config

def create_driver_skeleton(driver_class: str, instance_num: int) -> Tuple[str, Dict[str, Any]]:
    """Create a driver skeleton with all properties as comments or placeholders"""
    ...
    driver_config = {
        "name": name,
        "bindings": {}  # Will be populated with appropriate bindings
    }
...
def generate_yaml_with_comments(target_name: str, location: str, selected_resources: List[str], selected_drivers: List[str] = None, is_driver_file: bool = False) -> str:
...
def planner_main(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Generate YAML with helpful comments for resources
        if selected_resources:
            yaml_output = generate_yaml_with_comments(target_name, yaml_config[target_name]['location'], 
        ...
        if selected_drivers:
            ...
            driver_yaml_output = generate_yaml_with_comments(target_name, "", 
        ...
        print(f"\nGenerated configuration with skeletons:")
        print("=" * 60)
        print(yaml_output)
        print("=" * 60)
